...
/*
	...
	4. Query 실행
		...
		Statement 객체가 생성된 후에 
		Statement객체의 executeQuery(), executeUpdate() 메서드를 사용하여 처리한다.
		
		ResultSet rs=stmt.executeQuery("select * from emp where deptno=10");
		ResultSet rs=pstmt.executeQuery("select * from emp where deptno=?");
		ResultSet rs=cstmt.execute();
		
	5. ResultSet처리

		executeQuery()메서드는 결과를 ResultSet으로 리턴한다.
		이 ResultSet으로부터 읽은 데이터를 추출하는 과정이 ResultSet처리과정이다.
		
		데이터를 추출하는 방법은 ResultSet에서 한 행(row)씩 이동하면서 getXXX()메서드를 이용하여
		데이터를 추출하는데 rs.getString(1) or rs.getInt("컬럼명")의 방법으로 추출한다.
