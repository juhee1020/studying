...
<!--
	...
	out객체는 웹브라우저에 데이터를 전송하는 출력스트림으로서 jsp가 생성한 데이터를
	웹페이지에 출력한다.
		...
		2) out객체의 buffer 관련 메서드
			a. getBufferSize() : 버퍼의 크기를 리턴
			b. getRemaining()  : 버퍼의 남은 크기를 리턴
			c. clear()         : 버퍼전체를 비움, 이미 flush가 되어있다면 exception을 발생시킨다.
			d. clearBuffer()   : 버퍼전체 비움 (), 이미 flush가 되어있어도 exception 발생시키지않음.
			e. flush()         : 버퍼를 flush처리
			f. isAutoFlush()   : 자동 flush 여부를 boolean으로 리턴
...
<body>
	<% out.print("<b>안녕하세요! out기본객체입니다.</b>"); %>
	<% out.println("안녕하세요! out기본객체입니다."); %>
	<% out.print("<b>hello</b>"); %>
	<hr/>
	<h1>out기본객체의 buffer관련 메서드</h1>
