...
<!--
	JSP/Servlet은 웹애플리케이션이  사전에 정해진 규칙에 따라 특정폴더의 구조를 준수하도록 겅제화하고있다.
	이 구조를 모르면 제대로 동작하는 프로그램을 개발할 수 없게 된다.
	...
	3. WEB_INF/lib        : 웹애플리케이션에서 사용하는 라이브러리파일이 위치한다. (ex. ojdbc8.jar)
	...
	B. 웹애플리케이션의 배포(deploy)
		개발된 웹애플리케이션을 WAS(Web Application Server)에 배포하는 방법은 2가지가 있다.
			a. 개발된 프로그램의 폴더 그대로 파일을 WAS에 복사하는 방법 (톰캣서버폴더\webapps\ROOT 안에 풀면 된다.)
			b. war파일 형식으로 압축해서 배포하는 방법
				--- jdk의 jar.exe명령어를 사용해서 war파일을 생성
				--- 실행방법 : c:\>jar.exe cvf jsp01_basic.war *
				--- 위 명령 대신 이클립스 이용 : 프로젝트이름을 우클릭 > export > WAR file
...
<body>
	<h1>웹 Context 경로 = <%= request.getContextPath() %></h1>
</body>
</html>